...
export function estimateComplexity(source: { width: number; height: number }, settings: Pick<Settings, 'outputWidth' | 'outputHeight' | 'toolpathDetail' | 'units' | 'lineSpacing'>, mode: ConversionMode): ComplexityEstimate {
  ...
  const movements = mode === 'contour' ? Math.min(samples * 2, 500_000) : Math.min(samples, resolution.width * scanlines) + scanlines;
  ...
  while (recommendedDetail < 5) {
    ...
    const candidateMoves = mode === 'contour' ? Math.min(candidate.width * candidate.height * 2, 500_000) : Math.min(candidate.width * scanlines, candidate.width * candidate.height) + scanlines;
